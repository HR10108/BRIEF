...
from brief.get_grads.collect_grad_reps import (
    collect_grads,
    collect_reps,
    get_loss,
    get_knif,
)
from brief.get_grads.get_training_dataset import (
    get_dataloader,
...
def load_model(model_name_or_path: str, torch_dtype: Any = torch.bfloat16) -> Any:
    ...
    if (model_name_or_path.find("Qwen") != -1
...
if (
    args.info_type == "grads" or args.info_type == "knif"
...
if args.train_file is not None:
    ...
    # lm_datasets.column_names
    print(f"Dataset columns: {dataset.column_names}")
    if "messages" in dataset.features:
        dataset = dataset.remove_columns(["id", "messages","sample_id","source"])
